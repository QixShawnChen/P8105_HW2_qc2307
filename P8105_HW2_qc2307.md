...
``` r
wheel_clean_row = nrow(wheel_data_rounded)
wheel_clean_row
```
...
``` r
wheel_clean_col = ncol(wheel_data_rounded)
wheel_clean_col
```
...
`wheel_data_rounded` is generated after doing data clean and rounding
`sports_balls` variable. In the dataset wheel\_data\_rounded, there are
454 rows and 17 columns. The precipitation dataset
...
precipitation in 2018 is 70.33. The median precipitation in 2019 is
3.335.
...
```
    ...
    ##  6 WHITE NON HISPANIC         Olivia              248     NA     NA      1     NA
...
```
    ...
    ##  3 HISPANIC     Liam               387      1     NA     NA     NA     NA     NA
